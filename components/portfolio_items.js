...
const PortfolioItems = ({ portfolio_items }) => {
  ...
  const rightArticles = portfolio_items.slice(leftArticlesCount, portfolio_items.length);
      ...
      <div className="uk-child-width-1-2" data-uk-grid>
        <div>
          {leftArticles.map((portfolio_item, i) => {
            return <Card portfolio_item={portfolio_item} key={`article__${portfolio_item.id}`} />;
          })}
        </div>
        <div>
