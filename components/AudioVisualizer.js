...
const AudioVisualizer = (props) => {
    ...
    const [audioVisualizerInitialized, setAudioVisualizerInitialized] = React.useState(false);
    ...
    // set up audioContext
    useEffect(() => {
        setAudioContext(new window.AudioContext() || new window.webkitAudioContext())
    }, [props.playing])
    ...
    const draw = (localCtx, frameCount) => {
        ...
        if (!drawCurved) {
            ...
            localCtx.quadraticCurveTo(i * binWidth, canvasHeight - getBinHeight(i), (i + 1) * binWidth, canvasHeight - getBinHeight(i + 1));
            if (drawFilled) {
        ...
        if (drawText) {
            ...
            let localSongText = "";
            localCtx.fillText(songText, canvasWidth / 2 - textSize.width / 2, canvasHeight / 2 - 15 / 2 + 15);
    ...
    function setupAudioApi(audioElement) {
        ...
        localAudioAnalyserNode.fftSize = drawPitch ? displayBins * 8 : displayBins * 2;
        setMultiplier(Math.pow(22050, 1 / displayBins) * Math.pow(1 / magicConstant, 1 / displayBins));
        ...
        setLogBinLengths(List(createLookupTable(localAudioAnalyserNode.frequencyBinCount, localLogLookupTable)))
    ...
    return <>
        <Canvas draw={draw} opacity={0.2} drawing={drawing} onContextAvailable={(localCtx) => handleContextAvailable(localCtx)} />
    </>
};
